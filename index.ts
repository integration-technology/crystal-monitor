...
events.on('booking.scrape.request', async ({ body }) => {
  console.info('Starting booking scrape request for booking', body.Reference)
  const b = await Browser.start(true)
  const data = await Scraper.scrape(b, body)
  await Browser.close(b)
  await events.publish('booking.scrape.complete', {...data, ...{txnshash: hash.MD5(data.txns)}})
})
